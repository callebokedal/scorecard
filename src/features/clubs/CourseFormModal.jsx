...
export function CourseFormModal({ clubId, course, onClose }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!name.trim()) return;
    if (isEditing) {
      editCourse(clubId, course.id, { name: name.trim(), slope: Number(slope), note: note.trim() });
    } else {
      addCourse(clubId, { name: name.trim(), holes: Number(holes), slope: Number(slope), note: note.trim() });
    }
    onClose();
  };
  ...
  return (
    <Modal
      title={isEditing ? t('clubs.courseForm.editTitle') : t('clubs.courseForm.addTitle')}
      onClose={onClose}
    >
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            {t('clubs.courseForm.name')}
          </label>
          <input
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder={t('clubs.courseForm.namePlaceholder')}
            required
            autoFocus
          />
        </div>
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('clubs.courseForm.holes')}{' '}
              {isEditing && (
                <span className="text-gray-400 font-normal">{t('clubs.courseForm.holesLocked')}</span>
              )}
            </label>
            <select
        ...
        <div className="flex gap-3 pt-1">
          ...
          >
            {isEditing ? t('clubs.courseForm.saveBtn') : t('clubs.courseForm.addBtn')}
          </button>
        </div>
      </form>
    </Modal>
